...
export function buildDailyReportLines(): string[] {
  ...
  return [
    ...
    ...(d.pancake.count > 0
      ? [
          "",
          "On-chain P&L (Pancake, settled this GMT+7 day)",
      ...
      : [
          "",
          "On-chain P&L (Pancake, settled this GMT+7 day)",
          "  Net P&L   : — (no settled rows in pancake-placements.jsonl for this date)",
        ]),
    ...detailLines,
    "======================================================",
  ];
...
function buildDailyReportHeaderLinesHtml(d: DailyReportData): string[] {
  ...
  return [...base, ...buildPancakePlacementsSummaryHtmlLinesOrEmpty(d.pancake, "daily")];
}
