...
- After the merged **signal / pre-prediction** message, the bot sends **5** reminder Telegram messages, **1 second apart**, plain text: `**Signal Alert** 🔔` (no HTML parse mode), to draw attention.
...
Concrete numbers are in `src/config.ts`. The **default preset is relaxed** for live 5m BNB: a **higher** `maxAtrPct` (~5.5%) so violent drops/spikes are not rejected as “too chaotic” as often, **chopLookback 4** (harder to count as choppy), and a medium-loosen Exhaustion / Mirror preset vs the earlier strict tuning — **more signals, more noise**.
...
- `exhaustionUpRunMin=3`, `exhaustionUpRevBodyVsBaselineMult=0.62`, `exhaustionUpRevMinBodyToRange=0.3`
- `exhaustionDownRunMin=4`, `exhaustionDownRevBodyVsBaselineMult=0.48`, `exhaustionDownRevMinBodyToRange=0.38`
...
- Mirror UP context: `mirrorUpApplyChoppyVeto=false`, `mirrorUpMinEmaSlopeBars=4`, `mirrorUpMinEmaSlopePct=-0.008`, `mirrorUpBelowEmaLookback=10`, `mirrorUpMaxClosesBelowEma=7`, `mirrorUpMinGreenBodyVsPrevRedMult=0.9`
- `mirrorDownLightReconfirm=true`
- `mirrorDownMinBodyToRange=0.48`, `mirrorDownMaxImpulseRun=7`
